...
def readKnapsackInstance(directory, filename):
	values, weights = [], []
	n, wMax = 0, 0
	with open(directory + "/" + filename) as infile:
		for i, line in enumerate(infile):
			split = line.split()
			if n == 0 and wMax == 0:
				n, wMax = int(split[0]), int(split[1])
			elif i > n:
				break
			else:
				values.append(int(split[0]))
				weights.append(int(split[1]))
	with open(directory + "-optimum/" + filename) as infile:
		optimum = 0
		for line in infile:
			optimum = int(line)
		return n, wMax, values, weights, optimum
...
def getKnapsackSettings():
	algorithm = GA.runKnapsackGA
	maxGens = 200
	popsize = 30

	n, wMax, weights, values, targetscore = readKnapsackInstance(
		"../knapsack_instances/large_scale", "knapPI_1_100_1000_1")
	numCrossovers = popsize
	SCORES = {}

	genfunc = population.genPop
	genparams = (popsize, [population.genKnapsack], [(n, weights, wMax)])

	scorefunc = fitness.score
	scoreparams = ([fitness.scoreKnapsack], [(values,)], SCORES)
