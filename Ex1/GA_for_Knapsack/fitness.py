...
def score(p, scorefuncs, scorefuncparams, SCORES):
    ...
    if p not in SCORES:
        SCORES[p] = sum(scorefunc(chrom, *scoreparams) for scorefunc, scoreparams, chrom in
                        izip(scorefuncs, scorefuncparams, p.chromosomes))
...
def scoreKnapsack(knapsack, values):
    score = 0
    for i, char in enumerate(knapsack):
        if char == "1":
            score += values[i]
    return score